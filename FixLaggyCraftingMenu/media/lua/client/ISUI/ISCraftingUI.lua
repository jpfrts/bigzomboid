...
table.insertionSort = function (arr,compare)
    for i = 2, #arr, 1 do
        local tmp = arr[i]
        local j = i - 1
        while j >= 1 and compare(tmp, arr[j]) do
            arr[j+1] = arr[j]
            j = j - 1
        end
        arr[j+1] = tmp
...
function ISCraftingUI:refresh(repopulate)
    if repopulate == nil then repopulate = true end
    ...
    if repopulate then
        ...
        self:refreshRecipesListState();
    end
    

    self:sortList(repopulate);
...
ISCraftingUI.sortByAvailabilityAndName = function(a,b)
...
function ISCraftingUI:refreshRecipesListState()
    self.recipesList[getText("IGUI_CraftCategory_Favorite")] = {}
    
    self:getContainers();
    -- local availableItems = self:getAvailableItemsType();
    local count = 0;
    local filteredCount = 0;
    for category,itemList in pairs(self.recipesList) do
        local itemList = itemList;
        for _,item in ipairs(itemList) do
            if not item.evolved then
                count = count + 1;
                if self.character then
                   local mightAvailable = true;
                    -- for _,source in ipairs(item.sources) do
                    --     if #source.items == 1 and not availableItems[source.items[1].fullType] then
                    --         mightAvailable = false;
                    --         filteredCount = filteredCount +1;
                    --         break;
                    --     end
                    -- end
                    if mightAvailable then
                        item.available = RecipeManager.IsRecipeValid(item.recipe, self.character, nil, self.containerList);
                    end
                end
                if item.favorite then
                    table.insert(self.recipesList[getText("IGUI_CraftCategory_Favorite")], item);
                end
            end
        end
    end
    self.allRecipesListDirty = true;

    local oldAllRecipesList = self.allRecipesList;
    self.allRecipesList = {};
    for _,v in ipairs(oldAllRecipesList) do
        if not v.evolved then
            table.insert(self.allRecipesList,v);
        end
    end

    local oldCookingRecipesList = self.recipesList['Cooking'];
    self.recipesList['Cooking'] = {};
    for _,v in ipairs(oldCookingRecipesList) do
        if not v.evolved then
            table.insert(self.recipesList['Cooking'],v);
        end
    end
    self:populateCookingRecipesList();
    for _,cat in ipairs(self.categories) do
        if cat.filterAll:isSelected(1) then
            self:sortAllRecipesList();
            break;
        end
    end
...
function ISCraftingUI:sortAllRecipesList()
    if(self.allRecipesListDirty) then
        table.insertionSort(self.allRecipesList,self.sortByAvailabilityAndName);
            self.allRecipesListDirty = false;
    end
end
...
function ISCraftingUI:populateRecipesList()
    ...
    self:populateCookingRecipesList();
    table.sort(self.allRecipesList,self.sortByAvailabilityAndName);
end
...
function ISCraftingUI:sortList(repopulate) -- sort list with items you can craft in first
    if repopulate == nil then repopulate = true end
    ...
    for i,v in pairs(availableList) do
        if repopulate then
            ...
            table.insertionSort(v, ISCraftingUI.sortByName);
        end
