...
function ISCraftingCategoryUI:update()
    ...
    if self.parent.panel.activeView.view ~= self and filterAll then
        return;
    end
...
function ISCraftingCategoryUI:filter()
    if self.parent.panel.activeView.view ~= self and self.filteringAll then
        return;
    end
    if self.filteringAll then
        self.parent:sortAllRecipesList();
    end
    self.recipes:clear();
