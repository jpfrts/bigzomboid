...
function ISBoatMenu.onKeyStartPressed(key)
    ...
    if AquaConfig.isBoat(boat) then
        if getCore():getOptionPanCameraWhileDriving() then
            playerObj:getModData()["PanCameraWhileDriving"] = true
            getCore():setOptionPanCameraWhileDriving(false)
        end
    elseif playerObj:getModData()["PanCameraWhileDriving"] then
        getCore():setOptionPanCameraWhileDriving(true)
        playerObj:getModData()["PanCameraWhileDriving"] = nil
    end
...
function ISBoatMenu.onExit(playerObj)
    ...
    if AquaConfig.isBoat(boat) then
        local inCabin = boat:getPartById("InCabin" .. string.sub(boat:getPartForSeatContainer(boat:getSeat(playerObj)):getId(), 5))
        if not inCabin then
...
function ISBoatMenu.recoveryKeys(playerObj)
    ...
    if playerObj:getModData()["blockBackward"] then
        getCore():addKeyBinding("Backward", playerObj:getModData()["blockBackward"])
        playerObj:getModData()["blockBackward"] = nil
    end
    if playerObj:getModData()["blockStartVehicleEngine"] then
        getCore():addKeyBinding("StartVehicleEngine", playerObj:getModData()["blockStartVehicleEngine"])
        playerObj:getModData()["blockStartVehicleEngine"] = nil
    end
end
