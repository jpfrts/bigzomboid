...
function ISBoatSeatUI:render()
    ...
    if self.characterSeat then
        if self.vehicle:getPartById("InCabin" .. string.sub(self.vehicle:getPartForSeatContainer(self.vehicle:getSeat(self.character)):getId(), 5)) then
            playerInCabin = true
        end
    end
    for seat=1,self.vehicle:getMaxPassengers() do
        ...
        if self.vehicle:getPartById("InCabin" .. pngr:getId()) then
            isCabin = true
        end
        if posn then
...
function ISBoatSeatUI:useSeat(seat)
    ...
    if self.vehicle:getPartById("InCabin" .. self.vehicle:getScript():getPassenger(seat):getId()) then
        isCabin = true
    end
