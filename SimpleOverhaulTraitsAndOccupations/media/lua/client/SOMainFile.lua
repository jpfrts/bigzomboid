...
--				player:Say("today rain pain");
				SOAddPain(player, 100, "Head", (ZombRand(20)+20));
...
--			player:Say("tomorrow no rain pain");	
			SOAddPain(player, 25, "Head", ZombRand(32));
				if ZombRand(11) == 0 then
--				print("3");	
				player:playEmote("soheadache");
				end			
...
--				print("BThq stand: " ..  BTEndRestoringAmount)				
			...
			if (not player:isPlayerMoving() and player:isSitOnGround() == true) or player:getVehicle() ~= nil then	
...
function stronggtiptrait()
	...
	if player:HasTrait("DemoStrongGrip") then
		if player:getPrimaryHandItem() ~= nil then
			if player:getPrimaryHandItem():getType() == "Sledgehammer" then 
				local sledgehammer = player:getPrimaryHandItem();
				if SOMWActivated == 1 then
				sledgehammer:setEnduranceMod(0.7);
--				sledgehammer:setBaseSpeed(0.7);
...
--					sledgehammer:setBaseSpeed(1.2);
			...
			if player:getPrimaryHandItem():getType() == "Sledgehammer2" then
				local sledgehammer = player:getPrimaryHandItem();
				if SOMWActivated == 1 then 
				sledgehammer:setEnduranceMod(1.0);
--				sledgehammer:setBaseSpeed(1.1);
--				sledgehammer:setCantAttackWithLowestEndurance(false);	
				sledgehammer:setTooltip(getText("Tooltip_SOTO_StrongGripSledgehammer"));						
				else
					sledgehammer:setEnduranceMod(1.0);
    ...
    if player:HasItem("Sledgehammer") == true or player:HasItem("Sledgehammer2") == true then	
		...
		if skip == false then
			for i = 0, inventory:getItems():size() - 1 do
				...
				if item:getType() == "Sledgehammer" and  item:getTooltip() == getText("Tooltip_SOTO_StrongGripSledgehammer") then
					local sledgehammer = item;				
					if SOMWActivated == 1 then
					sledgehammer:setEnduranceMod(1);
--					sledgehammer:setBaseSpeed(0.6);	
...
--						sledgehammer:setBaseSpeed(0.9);	
				...
				if item:getType() == "Sledgehammer2" and item:getTooltip() == getText("Tooltip_SOTO_StrongGripSledgehammer") then
					local sledgehammer = item;				
					if SOMWActivated == 1 then
					sledgehammer:setEnduranceMod(2);
--					sledgehammer:setBaseSpeed(0.9);	
--					sledgehammer:setCantAttackWithLowestEndurance(true);			
					sledgehammer:setTooltip(null);	
					break;					
...
function caloriestraits()
	...
	if player:isAsleep() then
		...
		if player:HasTrait("AccMetabolism") then -- Losing weight faster when weight over 70
			if weight >= 84 then -- loses weight faster when over 82+ weight
				player:getNutrition():setCalories(currcalories - (CalSleeping * CalMod))
			end
			if weight <= 76 then -- gains weight faster when below 78- weight
				player:getNutrition():setCalories(currcalories + (CalSleeping * CalMod))
			end
		end
	end
	...
	if not player:isAsleep() then
		...
		if player:isPlayerMoving() and player:IsRunning() == false and player:isSprinting() == true then
			...
			if player:HasTrait("AccMetabolism") then -- Losing weight faster when weight over 70
				if weight >= 84 then -- gain weight slower when over 82+ weight
					player:getNutrition():setCalories(currcalories - (CalSprinting * CalMod))
				end
				if weight <= 76 then -- lose weight slower when below 78- weight
					player:getNutrition():setCalories(currcalories + (CalSprinting * CalMod))
				end
			end	
		end		
...
function caloriestraitsswing(player, weapon)
	...
	if player:HasTrait("AccMetabolism") then -- Losing weight faster when weight over 70
		if weight >= 84 then -- loses weight faster when over 85+ weight
			player:getNutrition():setCalories(currcalories - (calswingcost * calmod))
		end
		if weight <= 76 then -- gains weight faster when below 80- weight
			player:getNutrition():setCalories(currcalories + (calswingcost * calmod))
		end
	end
end
