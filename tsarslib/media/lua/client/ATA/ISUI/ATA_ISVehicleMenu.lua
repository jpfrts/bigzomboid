...
function ATA_ISVehicleMenu.getVehicleAtRearOfTrailer(trailer)
    ...
    for i=0, 8, 0.5 do    
        ...
        if vehicle then
            if vehicle:getPartById("TCLConfig") and vehicle:getPartById("TCLConfig"):getTable("TCLConfig") then
                if vehicle:getPartById("TCLConfig"):getTable("TCLConfig").wreckerName == trailer:getScript():getName() then
                    return vehicle
                end
            end
            if ATA2WreckerTable[trailer:getScript():getName()] and ATA2WreckerTable[trailer:getScript():getName()][vehicle:getScript():getName()] then
                return vehicle
            end
        end
    end
